...
# CONFIGURATION
# ======================
CHECKPOINT_DIR = "./checkpoints_Treebind_Augmented2"
TREE_BINDING_DIR = "./tree_binding_data_Augmented2"
...
TREE_CONFIG = {
    ...
    'backtracking_threshold': 0.3,  # Accuracy drop threshold for backtracking
    ...
    'plateau_aggressive_factor': 0.25,  # How aggressive to increase LR
...
class BacktrackingLROptimizer:
    ...
    def enter_aggressive_mode(self):
        ...
        print(f"  ⚡ Entering AGGRESSIVE MODE due to plateau")
        ...
        print(f"  ↳ Base LR increased from {old_base_lr:.6f} to {self.base_lr:.6f}")
    
    def exit_aggressive_mode(self):
        """Exit aggressive mode (called when improvement resumes)"""
        if self.aggressive_mode:
            self.aggressive_mode = False
            print(f"  ↪ Exiting aggressive mode")
    ...
    def apply_backtracking_adjustment(self, previous_best_accuracy, current_best_accuracy):
        ...
        if accuracy_drop > self.config['backtracking_threshold']:
            ...
            print(f"  🔙 BACKTRACKING triggered: Accuracy dropped {accuracy_drop:.2f}%")
            ...
            print(f"  ↳ Base LR decreased from {old_base_lr:.6f} to {self.base_lr:.6f}")
            return True
        return False
...
class BacktrackingTreeBindingManager:
        ...
        for idx, model_info in enumerate(self.population):
            ...
            for batch_idx in range(batches_to_train):
                ...
                if idx == 0 and batch_idx % 50 == 0:
                    ...
                    mode = "⚡" if optimizer.aggressive_mode else "○"
    ...
    def evaluate_population(self, val_loader, criterion):
        ...
        if current_best > self.best_accuracy:
            ...
            if self.aggressive_evolution_active:
                self.aggressive_evolution_active = False
                print(f"  ↪ Exiting aggressive evolution mode")
        else:
    ...
    def activate_aggressive_evolution(self):
        ...
        if not self.aggressive_evolution_active:
            ...
            print(f"\n  ⚡ ACTIVATING AGGRESSIVE EVOLUTION (plateau for {self.plateau_generations} gens)")
    ...
    def _aggressive_evolution(self):
        ...
        print(f"  ✓ Aggressive evolution: Increased mutation rate and LRs")
        return True
    ...
    def _backtrack_evolution(self, backtrack_info):
        ...
        print(f"  ✓ Backtracking evolution: Restored from gen {backtrack_info['best_historical_gen']}")
        print(f"  ↳ Created careful mutations with DECREASED LRs for precise adjustments")
        return True
...
def main():
    ...
    info_text = (
        ...
        f"LR Strategy:\n"
        f"  Plateau → Increase LR ({TREE_CONFIG['plateau_aggressive_factor']}x)\n"
        f"  Backtrack → Decrease LR ({TREE_CONFIG['backtracking_lr_factor']}x)"
    )
