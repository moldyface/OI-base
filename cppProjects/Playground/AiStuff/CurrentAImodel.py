...
# CONFIGURATION
# ======================
CHECKPOINT_DIR = "./checkpoints_Treebind_curr"
TREE_BINDING_DIR = "./tree_binding_data_curr"
...
TREE_CONFIG = {
    ...
    'high_acc_lr_factor': 0.25,
    'low_acc_lr_factor': 1.2,
    ...
    'high_velocity_lr_factor': 1.5, # Increase LR when velocity is high
    'low_velocity_lr_factor': 0.7,  # Decrease LR when velocity is low
...
class VelocityLROptimizer:
    ...
    def __init__(self, model, base_lr=None, lr_ratios=None, config=TREE_CONFIG):
        ...
        print(f"  High Velocity (> {config['high_velocity_threshold']}%/gen): LR×{config['high_velocity_lr_factor']}")
        print(f"  Low Velocity (< {config['low_velocity_threshold']}%/gen): LR×{config['low_velocity_lr_factor']}")
    ...
    def adjust_lr_based_on_velocity(self, current_accuracy):
        ...
        if adjustment_factor != 1.0:
            ...
            print(f"  {direction} LR {reason}{velocity_str}: {old_base_lr:.6f}→{self.base_lr:.6f} (×{adjustment_factor:.2f})")
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        if tree_accuracy > val_accuracy:
            ...
            print(f"  🔄 FORCING replacement of main model with tree's best")
            ...
            print(f"  ✓ Main model replaced with Tree Binding best")
            ...
            print(f"  ✓ Main model is better or equal ({val_accuracy:.2f}% vs {tree_accuracy:.2f}%)")
